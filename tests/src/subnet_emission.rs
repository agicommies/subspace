...
//         run_to_block(1); // run to next block to ensure weights are set on nodes after their
// registration block
...
//         let validator_uid = SubspaceMod::get_uid_for_key(yuma_netuid,
// &yuma_validator_key).unwrap();         let uid = [miner_uid].to_vec();
//         let weight = [1].to_vec();
